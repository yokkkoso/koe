...
@Injectable()
export class PrivateMuteController {
	public constructor (
		private readonly privatesService: PrivatesService,
	) {}

	@Button(`privateAction/${PrivateButtonType.MUTE}`)
	public async onMuteButton (
		...
		if (!await this.privatesService.isUserHavePrivateChannel(interaction.guildId!, interaction.user.id)) {
			await interaction.reply({
				embeds: [
					baseEmbed()
						.setTitle(PrivateButtonStrings[PrivateButtonType.MUTE])
	...
	@MentionableSelect('privateMuteChoose')
	public async onMentionableSelect (
		...
		if (selectedPermissions && !selectedPermissions.has('Speak', false)) {
			await interaction.update({
				embeds: [
					baseEmbed()
						.setTitle(PrivateButtonStrings[PrivateButtonType.MUTE])
						.setThumbnail(interaction.user.displayAvatarURL({ extension: 'png' }))
						.setDescription(`${interaction.user.toString()}, У ${selected.toString()} **уже нет права говорить** в Вашей приватной комнате`),
				],
				components: [],
			});
		...
		await interaction.update({
			embeds: [
				baseEmbed()
					.setTitle(PrivateButtonStrings[PrivateButtonType.MUTE])
					.setThumbnail(interaction.user.displayAvatarURL({ extension: 'png' }))
					.setDescription(`${interaction.user.toString()}, Вы успешно **забрали право говорить** ${'name' in selected ? `всем пользователям роли ${selected.toString()}` : `пользователю ${selected.toString()}`} в Вашей приватной комнате`),
			],
			components: [],
		});
	}
