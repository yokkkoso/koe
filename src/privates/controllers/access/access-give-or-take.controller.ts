...
@Injectable()
export class PrivateAccessGiveOrTakeController {
	public constructor (
		private readonly privatesService: PrivatesService,
	) {}

	@Button(`privateAction/${PrivateButtonType.ACCESS_GIVE_TAKE}`)
	public async onAccessGiveOrTakeButton (
		...
		await interaction.reply({
			embeds: [
				baseEmbed()
					.setTitle(PrivateButtonStrings[PrivateButtonType.ACCESS_GIVE_TAKE])
					.setThumbnail(interaction.user.displayAvatarURL({ extension: 'png' }))
					.setDescription(`${interaction.user.toString()}, **выберите** пользователя или роль, которому(ой) Вы хотите **выдать** или **забрать** доступ в Вашу комнату`),
			],
			components: [
				new ActionRowBuilder<MentionableSelectMenuBuilder>()
					.addComponents(
						new MentionableSelectMenuBuilder()
							...
							.setCustomId('privateAccessGiveOrTakeChoose')
							.setMaxValues(1),
					),
			],
			ephemeral: true,
	...
	@MentionableSelect('privateAccessGiveOrTakeChoose')
	public async onMentionableSelect (
		...
		const isGivenAccess = voiceChannel.permissionsFor(selected.id, false)?.has('Connect', false);

		if (isGivenAccess && 'username' in selected && selected.id === interaction.user.id) {
			await interaction.update({
				embeds: [
					baseEmbed()
		...
		await voiceChannel.permissionOverwrites.edit(selected.id, {
			Connect: !isGivenAccess,
			ViewChannel: !isGivenAccess,
		}, {
		...
		await interaction.update({
			embeds: [
				baseEmbed()
					.setTitle(isGivenAccess ? PrivateButtonStrings[PrivateButtonType.ACCESS_TAKE] : PrivateButtonStrings[PrivateButtonType.ACCESS_GIVE])
					.setThumbnail(interaction.user.displayAvatarURL({ extension: 'png' }))
					.setDescription(`${interaction.user.toString()}, Вы успешно **${isGivenAccess ? 'сняли' : 'выдали'} доступ** ${'name' in selected ? `всем пользователям роли ${selected.toString()}` : `пользователю ${selected.toString()}`} в Вашу приватную комнату`),
			],
			components: [],
		});
	}
