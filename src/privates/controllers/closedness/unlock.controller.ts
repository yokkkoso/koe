...
@Injectable()
export class PrivateUnlockController {
	public constructor (
		private readonly privatesService: PrivatesService,
	) {}

	@Button(`privateAction/${PrivateButtonType.UNLOCK}`)
	public async onUnlockButton (
		...
		if (!privateChannel) {
			await interaction.reply({
				embeds: [
					baseEmbed()
						.setTitle(PrivateButtonStrings[PrivateButtonType.UNLOCK])
		...
		if (voiceChannel.permissionsFor(interaction.guild!.roles.everyone.id, false)?.has('Connect', false)) {
			await interaction.reply({
				embeds: [
					baseEmbed()
						.setTitle(PrivateButtonStrings[PrivateButtonType.UNLOCK])
						.setThumbnail(interaction.user.displayAvatarURL({ extension: 'png' }))
						.setDescription(`${interaction.user.toString()}, Ваша приватная комната **уже** открыта для всех`),
				],
				ephemeral: true,
			});
		...
		await interaction.reply({
			embeds: [
				baseEmbed()
					...
					.setThumbnail(interaction.user.displayAvatarURL({ extension: 'png' }))
					.setDescription(`${interaction.user.toString()}, Вы успешно **открыли** Вашу приватную комнату для всех`),
			],
			ephemeral: true,
		});
	}
